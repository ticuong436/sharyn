...
export class MenuService {
  ...
  deleteMenu(id){
    return this.menu$.pipe(
      take(1),
      switchMap(menus=>this.http.delete(`http://localhost:3000/menu/${id}`).pipe(map((isDelete => {
  ...
  updateMenu(data){
    return this.menu$.pipe(
      take(1),
      switchMap(menus => this.http.patch(`http://localhost:3000/menu/${data.id}`,data).pipe(
          map((updateMenu) => {
